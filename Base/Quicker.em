...
/*            InsBufLine(hbuf, ln + 4, "@szLine1@" # "else");
	...
	else if (szCmd == "driver")
	{
        DelBufLine(hbuf, ln)
        CreateDriverDef(hbuf,szMyName,1)		
	}	
    else
    {
        SearchForward()
...
macro InsertFileHeaderCN(hbuf, ln,szName,szContent)
{
    ...
    InsBufLine(hbuf, ln + 0,  "/******************************************************************************")
    InsBufLine(hbuf, ln + 1,  "")
    InsBufLine(hbuf, ln + 2,  "                  版权所有 (C), 2015-2020, 北京佰才邦技术有限公司")
    InsBufLine(hbuf, ln + 3,  "")
    InsBufLine(hbuf, ln + 4,  " ******************************************************************************")
    sz = GetFileName(GetBufName (hbuf))
...
macro FuncHeadCommentEN(hbuf, ln, szFunc, szMyName,newFunc)
{
    ...
    if ((newFunc == 1) && (strlen(szFunc)>0))
    {
        InsBufLine(hbuf, ln+8, "UINT32_T  @szFunc@( # )")
    ...
    if ((newFunc == 1) && (strlen(szFunc)>0))
    {
        ...
        if(strlen(szRet) > 0)
        {
            ...
            PutBufLine(hbuf, ln+8, "@szRet@ @szFunc@( # )")
            SetbufIns(hbuf,ln+8,strlen(szRet)+strlen(szFunc) + 3
        }
        szFuncDef = ""
        isFirstParam = 1
        sel.ichFirst = strlen(szFunc)+strlen(szRet) + 3
...
macro CreateKmodDef(hbuf, szName, language)
{
	...
	InsBufLine(hbuf, ln + 15, "module_init(@szModName@_init);")
	InsBufLine(hbuf, ln + 16, "")
...
macro CreateDriverDef(hbuf, szName, language)
{
    hnewbuf = newbuf("")
    if(hnewbuf == hNil)
    {
    ...
    if(language == 0 )
    {
        szDriverName = Ask("请输入驱动名称:")
    }
    else
    {
        szDriverName = Ask("Please input driver name")
    }

	ln = 0
    InsBufLine(hbuf , ln + 0 , "/*")
    InsBufLine(hbuf , ln + 1 , " * Source code for driver @szDriverName@")
    InsBufLine(hbuf , ln + 2 , " *")
    InsBufLine(hbuf , ln + 3 , " * Copyright (C) 2018 Baicells Corporation")
    InsBufLine(hbuf , ln + 4 , " *")
    InsBufLine(hbuf , ln + 5 , " */")
    ...
    InsBufLine(hbuf , ln + 9 , "")
    InsBufLine(hbuf , ln + 10, "")
	InsBufLine(hbuf , ln + 11, "static int @szDriverName@_open(struct inode *inode, struct file *file)")
	InsBufLine(hbuf , ln + 12, "{")
	InsBufLine(hbuf , ln + 13, "    return 0;")
	InsBufLine(hbuf , ln + 14, "}")
	InsBufLine(hbuf , ln + 15, "")
	InsBufLine(hbuf , ln + 16, "static loff_t @szDriverName@_lseek(struct file *file, loff_t offset, int orig)")
	InsBufLine(hbuf , ln + 17, "{")
	InsBufLine(hbuf , ln + 18, "    return 0;")
	InsBufLine(hbuf , ln + 19, "}")
	InsBufLine(hbuf , ln + 20, "")
	InsBufLine(hbuf , ln + 21, "static ssize_t @szDriverName@_read(struct file *file, char __user *buf, size_t nbytes, loff_t *pos)")
	InsBufLine(hbuf , ln + 22, "{")
	InsBufLine(hbuf , ln + 23, "    return 0;")
	InsBufLine(hbuf , ln + 24, "}")
	InsBufLine(hbuf , ln + 25, "")
	InsBufLine(hbuf , ln + 26, "static ssize_t @szDriverName@_write(struct file *file, const char __user *buf, size_t nbytes, loff_t *ppos)")
	InsBufLine(hbuf , ln + 27, "{")
	InsBufLine(hbuf , ln + 28, "    return 0;")
	InsBufLine(hbuf , ln + 29, "}")
	InsBufLine(hbuf , ln + 30, "")
	InsBufLine(hbuf , ln + 31, "static int @szDriverName@_release(struct inode *inode, struct file *file)")
	InsBufLine(hbuf , ln + 32, "{")
	InsBufLine(hbuf , ln + 33, "    return 0;")
	InsBufLine(hbuf , ln + 34, "}")
	InsBufLine(hbuf , ln + 35, "")
	InsBufLine(hbuf , ln + 36, "static const struct file_operations @szDriverName@_ops = {")
	InsBufLine(hbuf , ln + 37, "    .owner		= THIS_MODULE,")
	InsBufLine(hbuf , ln + 38, "    .open		= @szDriverName@_open,")
	InsBufLine(hbuf , ln + 39, "    .llseek		= @szDriverName@_lseek,")
	InsBufLine(hbuf , ln + 40, "    .read		= @szDriverName@_read,")
	InsBufLine(hbuf , ln + 41, "    .write		= @szDriverName@_write,")
	InsBufLine(hbuf , ln + 42, "    .release	= @szDriverName@_release,")
	InsBufLine(hbuf , ln + 43, "};")
	InsBufLine(hbuf , ln + 44, "")
	InsBufLine(hbuf , ln + 45, "static int @szDriverName@_probe(struct platform_device *pdev)")
	InsBufLine(hbuf , ln + 46, "{")
	InsBufLine(hbuf , ln + 47, "    return 0;")
	InsBufLine(hbuf , ln + 48, "}")
	InsBufLine(hbuf , ln + 49, "")
	InsBufLine(hbuf , ln + 50, "static int @szDriverName@_remove(struct platform_device *pdev)")
	InsBufLine(hbuf , ln + 51, "{")
	InsBufLine(hbuf , ln + 52, "    return 0;")
	InsBufLine(hbuf , ln + 53, "}")
	InsBufLine(hbuf , ln + 54, "")
	InsBufLine(hbuf , ln + 55, "static int @szDriverName@_suspend(struct platform_device *dev, pm_message_t pm)")
	InsBufLine(hbuf , ln + 56, "{")
	InsBufLine(hbuf , ln + 57, "    return 0;")
	InsBufLine(hbuf , ln + 58, "}")
	InsBufLine(hbuf , ln + 59, "")
	InsBufLine(hbuf , ln + 60, "static int @szDriverName@_resume(struct platform_device *pdev)")
	InsBufLine(hbuf , ln + 61, "{")
	InsBufLine(hbuf , ln + 62, "    return 0;")
	InsBufLine(hbuf , ln + 63, "}")
	InsBufLine(hbuf , ln + 64, "")
	InsBufLine(hbuf , ln + 65, "static struct platform_driver @szDriverName@_driver = {")
	InsBufLine(hbuf , ln + 66, "    .probe		= @szDriverName@_probe,")
	InsBufLine(hbuf , ln + 67, "    .remove		= @szDriverName@_remove,")
	InsBufLine(hbuf , ln + 68, "    .suspend	= @szDriverName@_suspend,")
	InsBufLine(hbuf , ln + 69, "    .resume		= @szDriverName@_resume,")
	InsBufLine(hbuf , ln + 70, "    .id_table	= @szDriverName@_ids,")
	InsBufLine(hbuf , ln + 71, "    .driver		= {")
	InsBufLine(hbuf , ln + 72, "        .name	= \"@szDriverName@\",")
	InsBufLine(hbuf , ln + 73, "    },")
	InsBufLine(hbuf , ln + 74, "};")
	InsBufLine(hbuf , ln + 75, "")
	InsBufLine(hbuf , ln + 76, "static int __init @szDriverName@_init(void)")
	InsBufLine(hbuf , ln + 77, "{")
	InsBufLine(hbuf , ln + 78, "    return 0;")
	InsBufLine(hbuf , ln + 79, "}")
	InsBufLine(hbuf , ln + 80, "module_init(@szDriverName@_init);")
	InsBufLine(hbuf , ln + 81, "")
	InsBufLine(hbuf , ln + 82, "static void __exit @szDriverName@_exit(void)")
	InsBufLine(hbuf , ln + 83, "{")
	InsBufLine(hbuf , ln + 84, "    return;")
	InsBufLine(hbuf , ln + 85, "}")
	InsBufLine(hbuf , ln + 86, "module_exit(@szDriverName@_exit);")
	InsBufLine(hbuf , ln + 87, "")
	...
	InsBufLine(hbuf, ln + 88, "MODULE_AUTHOR(\"@szMyName@(Baicells)\");")
	InsBufLine(hbuf, ln + 89, "MODULE_DESCRIPTION(\"description for driver @szDriverName@\");")
	InsBufLine(hbuf, ln + 90, "MODULE_LICENSE(\"GPL\");")
}
